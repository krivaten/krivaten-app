...
import type { Relationship, RelationshipCreate, RelationshipUpdate } from "@/types/relationship";
...
export function useRelationships(entityId?: string) {
  ...
  const updateRelationshipMutation = useMutation({
    mutationFn: ({ id, updates }: { id: string; updates: RelationshipUpdate }) =>
      api.put<Relationship>(`/api/v1/relationships/${id}`, updates),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.relationships.all(),
      });
